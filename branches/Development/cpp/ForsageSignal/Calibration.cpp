...
class CalibrationDevice
{
	private:
		...
		wchar_t _pName[256];
		std::map<int, float> _calibratedValues;			// массив откалиброванных значений
		std::vector< std::pair<float, float> > vecCoefs;	// массив коэффициентов

		typedef std::map<int, float>::const_iterator CD_ITER;

		// проверяет значение val на вхождение в интервал между min и max
		static bool IsInRange(float min, float max, float val)
		{
			if (min <= max)
	...
	public:
		...
		int Calibrate()
		{
			...
			else
			{
				...
				float vmin, vmax, rmin, rmax;
				CD_ITER iter1 = _calibratedValues.begin();
				CD_ITER iter2 = iter1;
				std::pair<float, float> pairVal;
				++iter2;
		...
		void AddCalibrationData(int r, float v)
		{
			EnterCriticalSection(&objCS);
		...
		int SetDeviceValue(float v)
		{
			if(_enumState == CALIB_CALIB)
			{
				...
				CD_ITER iter1 = _calibratedValues.begin();
				CD_ITER iter2 = iter1;
				std::vector<std::pair<float, float> >::const_iterator iterC = vecCoefs.begin();
				++iter2;
				while (iter2 != _calibratedValues.end())
				{
				...
				float r = (iter1->second == iter2->second ? iter1->first : iterC->first*v + iterC->second);
				if (m_DeviceFun == NULL)
					return CALIBSAMPLE_ERRINIT;
				else
				{
					m_DeviceFun((int)r);
					return CALIBSAMPLE_NOERROR;
